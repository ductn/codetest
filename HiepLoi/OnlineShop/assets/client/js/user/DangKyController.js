...
var DangKyPortal = {
    init: function () {
        DangKyPortal.loadData();
        DangKyPortal.registerEvents();
    },
    registerEvents: function () {
        $("#fileUpload").on('change', function () {
            ...
            $.ajax({
                type: 'post',
                url: URL_APPLICATION + '/upFileFuction.ashx',
                data: formData,
                success: function (response) {
                    if (response != 'error') {
                        var obj = JSON.parse(response);
                        var tenFile = obj.tenFile;
                        var urlFile = obj.urlFile;
                        $("#myUploadedFile").attr("href", URL_APPLICATION + urlFile);
                        $("#nameFile").html(tenFile);
                        $("#urlFileDinhKem").val(urlFile);
                        $("#tenFileDinhKem").val(tenFile);
                    }
                },
                processData: false,
                contentType: false,
        ...
        $("#UserName").focusout(function () {
            ...
            DangKyPortal.getUserInfor(UserName);
        });
        ...
        $('#HuyenID').on('change', function () {
            ...
            var XaID = 0;
            DangKyPortal.getProvinceByParent(HuyenID, XaID);
        });
    },
                    ...
                    if (response.checkUser == false) {
                        ...
                        if (response.loai == "3") { // loai=3 -> Hộ kinh doanh
                            ...
                            if (data.ProvinceId != null) {
                                DangKyPortal.getProvinceByParent(data.ProvinceId, data.XaPhuongId);
                            }
                            ...
                            $('#HuyenID').val(data.ProvinceId);
                            $('#Address').val(response.sonha);
                            if (DangKyPortal.ProvinceId != null) {
                            ...
                            if (data.HuyenId != null) {
                                DangKyPortal.getProvinceByParent(data.HuyenId, data.PhuongId);
                            }
                        }
                    } else {
                        $("#UserName").focus();
