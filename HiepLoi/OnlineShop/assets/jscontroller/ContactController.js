...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-Contact", parent.document.body);
...
var Contact = {
    ...
    registerEvents: function () {
        ...
        $('.btnSave').off('click').on('click', function () {
            if ($('#formContact').valid()) {
                ...
                //if (Content == "") {
                //    bootbox.alert("Vui lòng nhập nội dung liên hệ!");
                //    $("#Content").focus();
                //    checkForm = 0;
                //    return false;
                //} else {
                ...
                //if (AnswerContent == "") {
                //    bootbox.alert("Vui lòng nhập nội dung trả lời liên hệ!");
                //    $("#AnswerContent").focus();
                //    checkForm = 0;
                //    return false;
                //} else {
        ...
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = Contact.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/Contact/ChangeContact",
                data: { id: id },
                dataType: "json",
                type: "POST",
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var controller = $(this).data('controller');
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-Contact\">";
            pupop += "<div class=\"modal-dialog modal-xl modal-dialog-centered\">";
            pupop += "<div class=\"modal-Contact\" >";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-Contact-view-Contact\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-Contact').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-Contact').modal();
        });
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/Contact/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(ContactConfig.pageSize);
    },
