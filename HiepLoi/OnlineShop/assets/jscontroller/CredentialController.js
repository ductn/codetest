...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-Credential", parent.document.body);
...
var Credential = {
    ...
    registerEvents: function () {
        ...
        $('.btnSave').off('click').on('click', function () {
            ...
            $('#sysThaotacdulieu input[type=checkbox]').each(function () {
                if (this.checked) {
                    abc = abc + $(this).val() + "|"
            ...
            if ($('#formPhanQuyen').valid()) {
                $("#formPhanQuyen").submit();
            }
        });
        $('#filter_count_perpage').off('change').on('change',function () {
            var slt = $(this).val();
            $.cookie('pageSize', slt, { path: '/' });
            var href = Credential.removeParams('page').replace('=undefined', '');
        ...
        $('#FunctionID').on('change', function () {
            var FunctionID = $('#FunctionID').val();
            var UserGroupID = $('#UserGroupID').val();
            Credential.getListRole(FunctionID, UserGroupID);
        });
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/Credential/ChangeCredential",
                data: { id: id },
                dataType: "json",
                type: "POST",
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-Credential\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-Credential\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-Credential').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-Credential').modal();
        });
        ...
        $('#btnDeleteAll').off('click').on('click', function () {
            ...
            if (values != "" && values != null) {
                ...
                bootbox.confirm("Bạn có chắc chắn muốn xóa danh sách bản ghi này không?", function (result) {
                    if (result == true) {
                        Credential.deleteAllData(values);
                    }
                });
            } else {
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/Credential/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    getListRole: function (FunctionID, UserGroupID) {
        var url = URL_APPLICATION + '/Admin/Credential/getListRole?FunctionID=' + FunctionID + '&UserGroupID=' + UserGroupID
        $.ajax({
            url: url,
            data: "",
            type: 'GET',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(CredentialConfig.pageSize);
    },
