...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-DeptIdentify", parent.document.body);
        ViewViecCuaToi.find('iframe').height(mainWrapperHeight);
    } catch (e) {
        // TODO: handle exception
    }
};
var DeptIdentifyConfig = {
    pageSize: $.cookie('pageSize') ? $.cookie('pageSize') : 5,
    pageIndex: 1,
}
var DeptIdentify = {
    init: function () {
        DeptIdentify.loadData();
        DeptIdentify.registerEvents();
    },
    registerEvents: function () {
        ...
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = DeptIdentify.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/DeptIdentify/ChangeStatus",
                data: { id: id },
                dataType: "json",
                type: "POST",
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-DeptIdentify\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-DeptIdentify\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-DeptIdentify').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-DeptIdentify').modal();
        });
        ...
        $('#btnDeleteAll').off('click').on('click', function () {
            ...
            if (values != "" && values != null) {
                ...
                bootbox.confirm("Bạn có chắc chắn muốn xóa danh sách bản ghi này không?", function (result) {
                    if (result == true) {
                        DeptIdentify.deleteAllData(values);
                    }
                });
            } else {
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/DeptIdentify/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(DeptIdentifyConfig.pageSize);
    },
