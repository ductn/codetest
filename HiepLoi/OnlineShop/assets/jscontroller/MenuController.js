...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-Menu", parent.document.body);
...
var Menu = {
    ...
    registerEvents: function () {
        $('#formMenu').validate({
            rules: {
                Text: { required: true, specialChars: true },
            },
            messages: {
                Text: { required: "Vui lòng nhập mã", specialChars: "Định dạng không hợp lệ" },
            }
        });
        ...
        $('#filter_count_perpage').off('change').on('change', function () {
            ...
            var href = Menu.removeParams('page').replace('=undefined', '');
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-Menu\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-Menu\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-Menu').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-Menu').modal();
        });
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/Menu/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(MenuConfig.pageSize);
    },
