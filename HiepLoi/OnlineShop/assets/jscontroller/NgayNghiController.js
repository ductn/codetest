...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-NgayNghi", parent.document.body);
...
var NgayNghi = {
    init: function () {
        NgayNghi.loadData();
        NgayNghi.registerEvents();
    },
    registerEvents: function () {
        $('#formNgayNghi').validate({
            rules: {
                Ngay: { required: true}
            },
            messages: {
                Ngay: { required: "Vui lòng nhập URL"}
            }
        }); 
        $('.btnSave').off('click').on('click', function () {
            if ($('#formNgayNghi').valid()) {
                $("#formNgayNghi").submit();
            }
        });
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = NgayNghi.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/NgayNghi/ChangeStatus",
                data: { id: id },
                dataType: "json",
                type: "POST",
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-NgayNghi\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-NgayNghi\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-NgayNghi').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-NgayNghi').modal();
        });
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/NgayNghi/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(NgayNghiConfig.pageSize);
    },
