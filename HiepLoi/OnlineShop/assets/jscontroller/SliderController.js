...
function sendFile(file) {
    ...
    var _url = URL_APPLICATION + '/Handler/SliderUploader.ashx?istype=Slider';
    $.ajax({
        type: 'post',
        url: _url,
        data: formData,
...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-Slider", parent.document.body);
...
var Slider = {
    ...
    registerEvents: function () {
        $('#formAction').validate({
            rules: {
                Url: { required: true, specialChars: true },
                Name: { required: true, specialChars: true }
            },
            messages: {
                Url: { required: "Vui lòng nhập url", specialChars: "Định dạng không hợp lệ" },
        ...
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = Slider.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-Slider\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-Slider\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-Slider').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-Slider').modal();
        });
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/Slider/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(SliderConfig.pageSize);
    },
