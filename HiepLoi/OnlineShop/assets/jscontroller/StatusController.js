...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-Status", parent.document.body);
...
var Status = {
    ...
    registerEvents: function () {
        $('#formTrangThai').validate({
            rules: {
                StatusId: { required: true, specialChars: true },
                Title: { required: true, specialChars: true }
            },
            messages: {
                StatusId: { required: "Vui lòng nhập mã", specialChars: "Định dạng không hợp lệ" },
                Title: { required: "Vui lòng nhập tên", specialChars: "Định dạng không hợp lệ" }
            }
        }); 
        $('.btnSave').off('click').on('click', function () {
            if ($('#formTrangThai').valid()) {
                $("#formTrangThai").submit();
            }
        });
        $('#filter_count_perpage').off('change').on('change',function () {
            var slt = $(this).val();
            $.cookie('pageSize', slt, { path: '/' });
            var href = Status.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-Status\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-Status\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-Status').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-Status').modal();
        });
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/Status/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(StatusConfig.pageSize);
    },
