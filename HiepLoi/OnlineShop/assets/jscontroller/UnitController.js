...
function sendFile(file) {
    ...
    $.ajax({
        type: 'post',
        url: URL_APPLICATION + '/Handler/SliderUploader.ashx?istype=Unit',
        data: formData,
        success: function (status) {
            if (status != 'error') {
                var my_path = status;
...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-Unit", parent.document.body);
...
var Unit = {
    ...
    registerEvents: function () {
        ...
        $('#filter_count_perpage').off('change').on('change', function () {
            ...
            var href = Unit.removeParams('page').replace('=undefined', '');
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-Unit\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-Unit\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-Unit').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-Unit').modal();
        });
        ...
        $('.btn-ShowOnHome').off('click').on('click', function (e) {
            ...
            var btn = $(this);
            var id = btn.data('id');
            Unit.ChangeShowOnHome(id, btn);
        });
    },
    ChangeShowOnHome: function (id, btn) {
        $.ajax({
            url: "/Admin/Unit/ChangeShowOnHome",
            data: { id: id },
            dataType: "json",
            type: "POST",
            success: function (response) {
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/Unit/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(UnitConfig.pageSize);
    },
