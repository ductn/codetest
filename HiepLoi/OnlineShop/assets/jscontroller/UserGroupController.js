...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-UserGroup", parent.document.body);
...
var UserGroup = {
    ...
    registerEvents: function () {
        ...
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = UserGroup.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/UserGroup/ChangeUserGroup",
                data: { id: id },
                dataType: "json",
                type: "POST",
                success: function (response) {
                    console.log(response);
                    if (response.UserGroup == true) {
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-UserGroup\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-UserGroup\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-UserGroup').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-UserGroup').modal();
        });
        ...
        $('#btnDeleteAll').off('click').on('click', function () {
            ...
            if (values != "" && values != null) {
                ...
                bootbox.confirm("Bạn có chắc chắn muốn xóa danh sách bản ghi này không?", function (result) {
                    if (result == true) {
                        UserGroup.deleteAllData(values);
                    }
                });
            } else {
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/UserGroup/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(UserGroupConfig.pageSize);
    },
