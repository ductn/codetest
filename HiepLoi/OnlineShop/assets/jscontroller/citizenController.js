...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-citizen", parent.document.body);
...
var citizen = {
    ...
    registerEvents: function () {
        ...
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = citizen.removeParams('page').replace('=undefined', '');
       ...
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/citizen/ChangeStatus",
                data: { id: id },
                dataType: "json",
                type: "POST",
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-citizen\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-citizen\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-citizen').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-citizen').modal();
        });
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION + '/Admin/citizen/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    loadData: function () {
        $('#filter_count_perpage').val(citizenConfig.pageSize);
    },
