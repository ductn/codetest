...
var autoSizeIframe = function autoSizeIframe() {
    try {
        ...
        var ViewViecCuaToi = $("#popup-view-sysAction", parent.document.body);
...
var sysAction = {
    init: function () {
        sysAction.loadData();
        sysAction.registerEvents();
    },
    registerEvents: function () {
        $('#formHanhDong').validate({
            rules: {
        ...
        $('.btnSave').off('click').on('click', function () {
            if ($('#formHanhDong').valid()) {
                $("#lstUserGroup").val($("#UserGroups").val());
                $("#formHanhDong").submit();
            }
        });
        $('#filter_count_perpage').off('change').on('change',function () {
            ...
            var href = sysAction.removeParams('page').replace('=undefined', '');
        ...
        $('#IdSysProcedure').on('change', function () {
            var IdSysProcedure = $('#IdSysProcedure').val();
            var url = window.location.href.replace(window.location.origin, "");
            if (url == '/Admin/SysAction' || url == '/Admin/SysAction/Index' || url == '/Admin/SysAction/index') {

            } else {
                sysAction.getListStatus(IdSysProcedure);
            }
        });
       /* $('.btn-active').off('click').on('click', function (e) {
            ...
            var id = btn.data('id');
            $.ajax({
                url: "/Admin/sysAction/ChangeStatus",
                data: { id: id },
                dataType: "json",
                type: "POST",
        ...
        $('.btn-view').off('click').on('click', function (e) {
            ...
            var pupop = "";
            pupop += "<div class=\"modal fade style-16\" id=\"popup-view-sysAction\">";
            ...
            pupop += "</div>";
            pupop += "<div class=\"modal-body\" id=\"popup-content-view-sysAction\" style='padding: 2px;'>";
            pupop += "<iframe src='" + controller + "' width='100%' height='500px' style='border:none;'></iframe>";
            pupop += "</div>";
            pupop += "</div>";
            pupop += "</div >";
            pupop += "</div >";
            $('#popup-view-sysAction').remove();
            $('.page-footer').append(pupop);
            $('#popup-view-sysAction').modal();
        });
        ...
        $('#btnDeleteAll').off('click').on('click', function () {
            ...
            if (values != "" && values != null) {
                ...
                bootbox.confirm("Bạn có chắc chắn muốn xóa danh sách bản ghi này không?", function (result) {
                    if (result == true) {
                        sysAction.deleteAllData(values);
                    }
                });
            } else {
    ...
    deleteAllData: function (ids) {
        $.ajax({
            url: URL_APPLICATION +'/Admin/sysAction/DeleteMulti',
            data: {
                ids: ids
            },
            type: 'POST',
    ...
    getListStatus: function (IdSysProcedure) {
        var url = URL_APPLICATION +'/Admin/SysAction/Create?IdSysProcedure=' + IdSysProcedure
        window.location = url;
        //$.ajax({
        //    url: url,
        //    data: "",
        //    type: 'GET',
        //    success: function (response) {
        //        $("#sysThaotacdulieu").html(response);
        //    },
        //    error: function (err) {
        //        console.log(err);
        //    }
    ...
    loadData: function () {
        $('#filter_count_perpage').val(sysActionConfig.pageSize);
    },
